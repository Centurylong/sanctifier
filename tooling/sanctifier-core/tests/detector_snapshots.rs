...
use sanctifier_core::rules::{
    ...
    sanct_unwrap::SanctUnwrapRule, timelock_parameter_change::TimelockParameterChangeRule,
...
#[test]
fn snapshot_timelock_parameter_change() {
    assert_detector_snapshot(
        "timelock_parameter_change",
        &TimelockParameterChangeRule::new(),
        include_str!("fixtures/detectors/timelock_parameter_change.rs"),
    );
}
...
#[test]
fn timelock_parameter_change_detector_is_registered_in_default_rules() {
    let findings = RuleRegistry::with_default_rules().run_by_name(
        include_str!("fixtures/detectors/timelock_parameter_change.rs"),
        "timelock_parameter_change",
    );
    ...
    assert!(findings
        .iter()
        .all(|finding| finding.rule_name == "SANCT_TIMELOCK_PARAMETER_CHANGE"));
}
