...
use sanctifier_core::rules::{
    ...
    sanct_unwrap::SanctUnwrapRule, symbol_short_length::SymbolShortLengthRule,
...
#[test]
fn snapshot_symbol_short_length() {
    assert_detector_snapshot(
        "symbol_short_length",
        &SymbolShortLengthRule::new(),
        include_str!("fixtures/detectors/symbol_short_length.rs"),
    );
}
...
#[test]
fn symbol_short_length_detector_is_registered_in_default_rules() {
    let findings = RuleRegistry::with_default_rules().run_by_name(
        include_str!("fixtures/detectors/symbol_short_length.rs"),
        "symbol_short_length",
    );
    ...
    assert!(findings
        .iter()
        .all(|finding| finding.rule_name == "SANCT_SYMBOL_SHORT_LENGTH"));
}
