...
use sanctifier_core::rules::{
    ...
    ledger_randomness::LedgerRandomnessRule, ledger_size::LedgerSizeRule,
...
#[test]
fn snapshot_ledger_randomness() {
    assert_detector_snapshot(
        "ledger_randomness",
        &LedgerRandomnessRule::new(),
        include_str!("fixtures/detectors/ledger_randomness.rs"),
    );
}
...
#[test]
fn ledger_randomness_detector_is_registered_in_default_rules() {
    let findings = RuleRegistry::with_default_rules().run_by_name(
        include_str!("fixtures/detectors/ledger_randomness.rs"),
        "ledger_randomness",
    );
    ...
    assert!(findings
        .iter()
        .all(|finding| finding.rule_name == "SANCT_LEDGER_RANDOMNESS"));
}
