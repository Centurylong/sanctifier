...
use sanctifier_core::rules::{
    ...
    unused_variable::UnusedVariableRule, view_state_write::ViewStateWriteRule, Rule, RuleRegistry,
};
...
#[test]
fn snapshot_view_state_write() {
    assert_detector_snapshot(
        "view_state_write",
        &ViewStateWriteRule::new(),
        include_str!("fixtures/detectors/view_state_write.rs"),
    );
}
...
#[test]
fn view_state_write_detector_is_registered_in_default_rules() {
    let findings = RuleRegistry::with_default_rules().run_by_name(
        include_str!("fixtures/detectors/view_state_write.rs"),
        "view_state_write",
    );
    ...
    assert!(findings
        .iter()
        .all(|finding| finding.rule_name == "SANCT_VIEW_STATE_WRITE"));
}
