...
pub const REINITIALIZATION_GUARD: &str = "SANCT_REINITIALIZATION_GUARD";
...
pub fn all_finding_codes() -> Vec<FindingCode> {
    vec![
        ...
        FindingCode {
            code: REINITIALIZATION_GUARD,
            category: "initialization",
            description:
                "Initialize entrypoint writes deployment-time state without an already-initialized guard",
        },
        FindingCode {
            code: SANCT_UNWRAP,
            category: "panic_handling",
...
mod tests {
    ...
    fn includes_expected_codes() {
        ...
        assert!(codes.iter().any(|c| c.code == REINITIALIZATION_GUARD));
