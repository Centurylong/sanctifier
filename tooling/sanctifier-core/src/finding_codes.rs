...
pub const TEMPORARY_PERSISTENT_STORAGE: &str = "SANCT_TEMPORARY_PERSISTENT_STORAGE";
...
pub fn all_finding_codes() -> Vec<FindingCode> {
    vec![
        ...
        FindingCode {
            code: TEMPORARY_PERSISTENT_STORAGE,
            category: "storage_lifetime",
            description: "Durable contract state is written to temporary storage",
        },
        FindingCode {
            code: SANCT_UNWRAP,
            category: "panic_handling",
...
mod tests {
    ...
    fn includes_expected_codes() {
        ...
        assert!(codes.iter().any(|c| c.code == TEMPORARY_PERSISTENT_STORAGE));
