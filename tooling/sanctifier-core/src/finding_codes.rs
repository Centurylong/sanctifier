...
pub const OWNERSHIP_TRANSFER: &str = "SANCT_OWNERSHIP_TRANSFER";
...
pub fn all_finding_codes() -> Vec<FindingCode> {
    vec![
        ...
        FindingCode {
            ...
            description:
                "Owner or admin control is transferred in one step without a pending accept flow",
        },
        FindingCode {
            code: SANCT_UNWRAP,
            category: "panic_handling",
...
mod tests {
    ...
    fn includes_expected_codes() {
        ...
        assert!(codes.iter().any(|c| c.code == OWNERSHIP_TRANSFER));
