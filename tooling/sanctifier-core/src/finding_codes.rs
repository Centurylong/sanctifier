...
pub const RESERVE_WITHDRAWAL_AUTH: &str = "SANCT_RESERVE_WITHDRAWAL_AUTH";
...
pub fn all_finding_codes() -> Vec<FindingCode> {
    vec![
        ...
        FindingCode {
            code: RESERVE_WITHDRAWAL_AUTH,
            category: "authentication",
            description:
                "Reserve or treasury withdrawal path is missing admin authorization or nonce guards",
        },
        FindingCode {
            code: SANCT_UNWRAP,
            category: "panic_handling",
...
mod tests {
    ...
    fn includes_expected_codes() {
        ...
        assert!(codes.iter().any(|c| c.code == RESERVE_WITHDRAWAL_AUTH));
