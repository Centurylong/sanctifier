...
pub const DYNAMIC_TARGET: &str = "SANCT_DYNAMIC_TARGET";
...
pub fn all_finding_codes() -> Vec<FindingCode> {
    vec![
        ...
        FindingCode {
            ...
            category: "unsafe_patterns",
            description:
                "Flag invoke targets whose Address/contract id flows from user arguments without an allowlist",
        },
    ]
}
...
mod tests {
    ...
    fn includes_expected_codes() {
        ...
        assert!(codes.iter().any(|c| c.code == DYNAMIC_TARGET));
    }
}
