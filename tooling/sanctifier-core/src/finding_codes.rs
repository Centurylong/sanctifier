...
pub const SANCT_EAGER_UNWRAP_OR: &str = "SANCT_EAGER_UNWRAP_OR";
...
pub fn all_finding_codes() -> Vec<FindingCode> {
    vec![
        ...
        FindingCode {
            code: SANCT_EAGER_UNWRAP_OR,
            category: "performance",
            description:
                "unwrap_or eagerly computes an expensive fallback that should be lazy",
        },
    ]
}
...
mod tests {
    ...
    fn includes_expected_codes() {
        ...
        assert!(codes.iter().any(|c| c.code == SANCT_EAGER_UNWRAP_OR));
    }
}
