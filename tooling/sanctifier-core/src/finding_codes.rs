...
pub const LEDGER_RANDOMNESS: &str = "SANCT_LEDGER_RANDOMNESS";
...
pub fn all_finding_codes() -> Vec<FindingCode> {
    vec![
        ...
        FindingCode {
            code: LEDGER_RANDOMNESS,
            category: "randomness",
            description:
                "Ledger sequence or timestamp is used as predictable randomness",
        },
        FindingCode {
            code: SANCT_UNWRAP,
            category: "panic_handling",
...
mod tests {
    ...
    fn includes_expected_codes() {
        ...
        assert!(codes.iter().any(|c| c.code == LEDGER_RANDOMNESS));
